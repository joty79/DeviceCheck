...
## 🔵 Shared WinRM Discovery

- Canonical discovery code is owned by `.agent-shared\modules\WinRMDiscovery`; DeviceCheck consumes its pinned `.assets\WinRMDiscovery` copy.
- Keep `Ctrl+L` rendering/keys, credentials, hardware snapshots, and local/remote target state inside DeviceCheck. Keep target history/catalog, short-lived LAN snapshots, WS-Discovery, Explorer Network enumeration, PC-port sweep, filtering, and name resolution in the shared module.
- Never edit the vendored module directly. Use `.agent-shared\scripts\Sync-WinRMDiscovery.ps1`, then run the DeviceCheck structure and live discovery tests.
...
### 2026-07-22 — Extract reusable discovery engine

- **Problem:** The proven `Ctrl+L` discovery code was needed by DiskHealth and future WinRM tools, creating copy/paste and drift risk.
- **Root cause:** Discovery was implemented only as DeviceCheck-internal functions.
- **Guardrail:** Route discovery/history/catalog through the shared module with the existing DeviceCheck state root. The first selector frame must not scan or bulk-validate saved targets; `Scan network now`/`R` is explicit and only the selected saved row is resolved. Keep the internal discovery implementation temporarily as a migration fallback.
- **Files affected:** `.assets\WinRMDiscovery\*`, `DeviceCheck.ps1`, `internal\DeviceCheck\06-RemoteConnection.ps1`, `README.md`, `CHANGELOG.md`, `PROJECT_RULES.md`.
- **Validation:** Shared module offline/live tests, DeviceCheck structure guard, and live expected-target test for `SE / 192.168.1.5`.

### 2026-07-27 — Portable VT validation and manifest line endings
...
Date: 2026-07-11
Problem: The AMD Chipset Software `8.05.04.516` outer package extracted 550 files and contained the real 20 MB `AMD_Chipset_Drivers.exe` child installer, but the preview reported a generic `Executable utility payload` with zero INF files.
Root cause: Some vendor packages are bootstrappers: static outer extraction exposes installer UI/runtime files while the final INF payload is unpacked only by a nested child process during execution. Treating every no-INF executable payload as a utility hides this distinction.
Guardrail/rule: When a no-INF extraction contains non-redistributable executables whose names or version metadata identify setup/install/driver/chipset behavior, classify it as `Nested installer/bootstrapper payload`. Preserve executable product/version/company metadata and state clearly that before/after snapshots plus SetupAPI evidence are authoritative. If the outer process spawns a child, do not capture the after snapshot until the child UI has fully completed.
Files affected: `tools\Trace-DriverPackageImpact.ps1`, `CHANGELOG.md`, `PROJECT_RULES.md`.
Validation/tests run: Parser validation; regenerated the existing AMD Chipset Software preview without running the installer; it now identifies nested product `AMD_Chipset_Drivers` version `8.05.04.516`, reports `Nested installer/bootstrapper payload`, and retains zero direct INF matches without mislabeling the package as a utility.

Date: 2026-07-11
Problem: Generic extractors exposed only the AMD chipset outer NSIS wrapper and failed to reveal the modern InstallShield payload, leaving the tracer with zero INF files. After extraction, the package's bare `USB\ROOT_HUB30` model entry also produced false exact matches against unrelated USB root hubs.
Root cause: The real payload is an InstallShield 29.x Basic MSI administrative image embedded behind `AMD_Chipset_Drivers.exe`. 7-Zip understands the outer NSIS layer but not the inner InstallShield overlay; legacy `IsXunpack` does not support this generation. Bare USB root-hub IDs are generic even when reported as hardware IDs rather than compatible IDs.
Guardrail/rule: Identify installer engines before selecting an extractor. Prefer vendor/native administrative extraction (`/a`) for modern InstallShield Basic MSI wrappers, verify that DriverStore, SetupAPI, and uninstall state did not change, then place the final payload under the tracer's `extracted\<base-name>` convention. Reject `USB\ROOT_HUB20`, `USB\ROOT_HUB30`, and bare `USB\ROOT_HUB` from preview matching while retaining vendor-specific VID/PID forms. Keep Procmon as a dynamic fallback, not the default, because even a short extraction can produce hundreds of thousands of events.
Files affected: `tools\Trace-DriverPackageImpact.ps1`, `docs\INSTALLER_ANALYSIS_TOOLKIT.md`, `README.md`, `CHANGELOG.md`, `PROJECT_RULES.md`.
Validation/tests run: Installed/tested 7-Zip 26.02, innoextract 1.9, Detect It Easy 3.21, lessmsi 2.12.9, WiX 3.14.1, Universal Extractor 2 RC3, Strings, and Procmon. InstallShield `/a` returned `0` and produced 414 files/68 INF/37 MSI/54 SYS/68 CAT with published-driver count `166 → 166`, SetupAPI growth `0`, and uninstall entries `2 → 2`. Regenerated the AMD preview: 68 INF, 30 candidate matches, zero bare `USB\ROOT_HUB30` matches, and two retained AMD-specific root-hub matches. Two inline audit commands initially hit the known bare-`foreach` pipeline parser error; both were rerun through named collections before using their results.

Date: 2026-07-11
Problem: The installer-toolkit summary said WiX 7 required OSMF EULA acceptance even for `--help`, but the actual command previously run was the narrower `wix msi decompile --help`.
Root cause: The result from a blocked subcommand-help invocation was generalized to root help without testing `wix --help` separately.
Guardrail/rule: Preserve the exact command boundary when reporting CLI restrictions. For WiX 7, root `wix --help` and `wix --version` work without EULA acceptance; `wix msi --help` and `wix msi decompile --help` return `WIX7015` unless the OSMF EULA is accepted for the invocation or persistently. Never accept a license/EULA on the user's behalf.
Files affected: `docs\INSTALLER_ANALYSIS_TOOLKIT.md`, `PROJECT_RULES.md`.
Validation/tests run: Administratively extracted the official WiX `7.0.0` CLI MSI into ignored test data without installing it, then ran all four commands against the same `wix.exe`: root help displayed usage/options, version returned `7.0.0+b8977d6`, and both MSI-level help commands returned `WIX7015`.

Date: 2026-07-11
Problem: WiX 7 could not be meaningfully compared with WiX 3 until its OSMF EULA boundary was explained and explicitly accepted, and the initial warning made the ordinary testing terms sound more obstructive than they were for this personal lab.
Root cause: Tool-install permission and legal-term acceptance are separate decisions, but the warning did not promptly explain the practical personal-testing impact or offer a per-command, non-persistent acceptance path.
Guardrail/rule: Warn the user early and visibly with `⚠️⚠️⚠️` whenever a tool requires EULA/Terms acceptance, summarize the practical boundary, and request explicit acceptance. After acceptance, prefer per-command/non-persistent acceptance when supported. For WiX 7, use `-acceptEula wix7`; do not create a persistent acceptance file unless explicitly requested.
Files affected: `docs\INSTALLER_ANALYSIS_TOOLKIT.md`, `CHANGELOG.md`, `PROJECT_RULES.md`.
Validation/tests run: WiX 7 MSI help succeeded with per-command acceptance. Side-by-side decompilation of the AMD main and AMD PCI MSI gave the same result in WiX 3 and WiX 7: exit `182`, proprietary `ISDRMFile` error, and matching extracted-stream counts (`24` and `22`). Both tools successfully decompiled the official WiX CLI control MSI and extracted `62` files, proving the test runtimes were functional.

Date: 2026-07-11
Problem: Adding automatic extractor orchestration directly to the already large standalone trace script would make future review, testing, and maintenance harder.
Root cause: `tools\Trace-DriverPackageImpact.ps1` already combines trace coordination, snapshots, preview matching, SetupAPI interpretation, verdicts, and report generation. A new multi-tool extraction subsystem would add unrelated routing and native-command complexity to the same file.
Guardrail/rule: Keep `Trace-DriverPackageImpact.ps1` as the single user-facing/context-menu coordinator, but implement new extraction identification, routing, caching, manifests, and tool adapters in focused internal helper files. Prefer a few cohesive files over either one oversized script or many tiny fragments. Preserve CLI parameters on the coordinator and do not integrate this standalone lab into the main DeviceCheck TUI.
Files affected: `PROJECT_RULES.md`; future standalone driver-trace extraction helper files.
Validation/tests run: Decision-only documentation change; implementation validation will include parser checks per helper, structure guard, focused extraction smokes, and `git diff --check`.

Date: 2026-07-11
Problem: The standalone tracer could analyze only payloads extracted manually beside an installer, even though proven tools were installed and engine-specific routing was now understood. Adding all routing directly to the 2,300-line coordinator would also make it harder to maintain. During implementation, a generic ProcessStartInfo argument path left hidden InstallShield processes waiting, optional uninstall Registry properties broke under StrictMode, a deep administrative target failed with MSI `Error 1304`, a strongly typed Markdown `List[string]` helper parameter was enumerated until it hit an empty line, and lessmsi treated a destination without trailing `\` as a filename filter and leaked test output into the repo root.
Root cause: Extraction had no modular adapter/cache/manifest layer; InstallShield requires its verified `/v"<MSI arguments>"` command-line shape; uninstall keys do not all expose display properties; legacy MSI file operations still hit practical path limits even on modern Windows; mutable generic collections passed between PowerShell functions can be unrolled during strongly typed parameter conversion; and lessmsi uses a trailing directory separator to distinguish its optional extraction destination from file filters.
Guardrail/rule: Keep `Trace-DriverPackageImpact.ps1` as the single coordinator/context-menu entry point and put extractor routing plus mutation guards in focused helpers. Default the context menu to `Safe`; identify with DIE; route Inno to innoextract, NSIS/generic containers to 7-Zip, and MSI to lessmsi; cache by source SHA-256; record every attempt in `extraction-manifest.json`; and offer `Extended` only for DIE-identified InstallShield with Strings proof of administrative-installation text plus embedded MSI evidence. Extended extraction must use the verified Start-Process argument form, a short `%TEMP%` administrative staging directory, elevation, and before/after DriverStore, SetupAPI, and uninstall guards. Optional Registry values and empty collections must be StrictMode-safe. When a helper mutates a caller-owned generic collection, accept the collection as one object reference instead of relying on parameter conversion that may enumerate its contents. Always pass lessmsi extraction destinations with a trailing `\` and verify that output stayed under the requested cache root.
Files affected: `tools\Trace-DriverPackageImpact.ps1`, `tools\DriverPackageTrace\PackageExtraction.ps1`, `tools\DriverPackageTrace\ExtractionGuard.ps1`, `tools\DriverPackageTrace\TraceExtractionCoordinator.ps1`, `tools\Launch-DriverPackageImpactTrace.vbs`, `Install-DriverPackageTraceContextMenu.ps1`, `README.md`, `docs\INSTALLER_ANALYSIS_TOOLKIT.md`, `CHANGELOG.md`, `PROJECT_RULES.md`.
Validation/tests run: Real Safe Inno extraction of `Realtek Audio Driver.exe` produced `1,250` files and `11` INF through innoextract; repeated helper invocation reused the SHA cache; official WiX control MSI routed through lessmsi with exit `0`, one successful attempt, `56` files under the requested cache, and no repo-root output leak; real AMD outer Safe extraction identified `NSIS → InstallShield`, ran two 7-Zip attempts, produced `604` combined files/zero INF, and retained one Extended candidate; elevated AMD nested Extended extraction completed with exit `0`, combined cache `468` files/`68` INF/`37` MSI/`54` SYS/`68` CAT, published drivers `166 → 166`, unchanged SetupAPI, and unchanged uninstall inventory. Full `PreviewOnly` tracer smokes produced extraction evidence and reports for both Realtek Safe and AMD Extended. A redirected-input context-flow smoke displayed `SAFE EXTRACTION INCOMPLETE`, accepted `N`, did not run Extended mode, and still wrote the Safe report. Context-menu installer parser passed; Registry readback confirmed only `SystemFileAssociations\.exe\shell`, with the dangerous legacy `exefile\shell` verb absent. Installed official PSScriptAnalyzer `1.25.0` from PSGallery and analyzed the five touched PowerShell files; zero analyzer errors, with correctness findings for automatic-variable naming, empty catches, and an unused variable fixed before final validation.

Date: 2026-07-11
Problem: OEM installer traces showed what Lenovo packages did locally, but DeviceCheck had no standalone evidence surface that could compare the active binding with current Windows Update offers, public Catalog discovery, the existing OEM trace, and SDIO without conflating their different ranking/applicability semantics.
Root cause: The existing driver-candidate tools produced search links and SDIO evidence separately. WUAPI applicability, MSCatalogLTS public rows, SetupAPI trace outcomes, and active PnP state had no normalized cross-source coordinator, while research reports also contained contradictory SDIO constants and unsupported claims.
Guardrail/rule: Keep driver-source comparison standalone from the main DeviceCheck TUI. Preserve each provider's native evidence and confidence: Windows live/SetupAPI rank is authoritative only for the observed local event; WUAPI proves only the current client search; Catalog rows are discovery-only until the actual package INF is inspected; SDIO score/status is never relabeled as Windows rank; and an OEM trace applies only to that installer run. Query ordered real Hardware IDs because a revision-specific Catalog search may return no rows while the next real device Hardware ID does. Do not recommend replacement when no verified better candidate exists.
Files affected: `tools\Compare-DriverSources.ps1`, `tools\DriverSourceComparison\*.ps1`, `internal\Test-DriverSourceComparison.ps1`, `docs\WINDOWS_DRIVER_SOURCE_COMPARISON_CONTRACT.md`, `README.md`, `CHANGELOG.md`, `PROJECT_RULES.md`.
Validation/tests run: Parser validation for the coordinator, five helpers, and focused test; focused comparison-model test; real local-only RZ616 smoke; real WUAPI search (`ResultCode 2`, one current driver offer overall, zero RZ616 matches); real MSCatalogLTS queries (revision-specific ID zero rows, subsystem ID 25 public rows); real Lenovo WLAN trace import (one exact preview match, active and outranked SetupAPI nodes); real SDIO audit-only run with `-disableinstall` (exit `0`, 247 devices, one RZ616 match, six candidates, active `25.40.2.586` labeled `SAME+CURRENT`, six preserved parser warnings); full combined JSON/Markdown report with `KeepCurrent_NoVerifiedBetterCandidate`; PSScriptAnalyzer, structure guard, line-ending audit, and `git diff --check` completed in final validation.

Date: 2026-07-11
Problem: Public Catalog rows for the RZ616 showed version `3.5.0.1392`, but title metadata alone could not establish the real INF `DriverVer`, exact device match, package signatures, payload differences, or whether the candidate would outrank the active SDIO-installed branch.
Root cause: Three 25H2 Catalog GUIDs shared one 8.1 MB CAB while another relevant GUID used a different 11.4 MB CAB. The public rows did not explain why the payloads differed or expose enough CHID/applicability evidence, and current WUAPI returned no RZ616 offer.
Guardrail/rule: Use MSCatalogLTS to obtain/download the actual Microsoft payload, verify the advertised SHA-1 and local SHA-256/signature status, extract with built-in `expand.exe`, parse the INF through the existing parser, and guard published-INF count, DriverStore folders, and SetupAPI length before/after. Bind extraction reuse to CAB SHA-256 plus a completion marker. Report computed FeatureScore/IdentifierScore/rank body only as `StrongInferenceNotWindowsSelectionEvidence`; actual Windows selection requires a separately authorized mutation-traced staging/update experiment. When exact signed candidates have a later date but WUAPI offers none, keep the current driver and label the candidate as requiring controlled selection proof rather than declaring it best.
Files affected: `tools\Inspect-CatalogDriverPackage.ps1`, `tools\Compare-DriverSources.ps1`, `tools\DriverSourceComparison\PackageInspection.ps1`, `tools\DriverSourceComparison\ComparisonReport.ps1`, `internal\Test-DriverSourceComparison.ps1`, `docs\WINDOWS_DRIVER_SOURCE_COMPARISON_CONTRACT.md`, `README.md`, `CHANGELOG.md`, `PROJECT_RULES.md`.
Validation/tests run: Windows 11 25H2 build `26200.8737`; MSCatalogLTS downloaded GUIDs `566cb1be-0030-4174-970c-6b6c159a34f2` and `9990c2a8-cab4-48df-ae4b-d5bc9a209f97`; both advertised SHA-1 hashes matched and CAB/CAT/SYS signatures were valid; both INFs exactly matched `PCI\VEN_14C3&DEV_0616&SUBSYS_E0C617AA`, exposed `DriverVer 03/24/2026, 3.05.00.1392`, default FeatureScore `0xFF`, IdentifierScore `0x0001`, and computed rank body `00FF0001`; guards stayed `166 → 166` published INFs, `884 → 884` DriverStore folders, and `2043 → 2043` SetupAPI bytes. Final validation also covered hash-bound extraction reuse, parser/tests, PSScriptAnalyzer, structure guard, line endings, `git diff --check`, and a full combined comparison report.

Date: 2026-07-12
Problem: Το inspected RZ616 Catalog package είχε computed rank `00FF0001`, αλλά το report δεν μπορούσε να αποδείξει αν τα Windows θα το προτιμούσαν χωρίς να γίνει risky Wi-Fi driver activation.
Root cause: Offline INF parsing είναι strong inference, ενώ authoritative local selection status εμφανίζεται μόνο αφού το package γίνει staged και αξιολογηθεί από το Windows PnP. Actual `/install` θα μπορούσε να κάνει προσωρινό disconnect του μοναδικού Wi-Fi connection και να διακόψει την Codex task.
Guardrail/rule: Για network drivers, χωρίζουμε το selection proof από το activation proof. Επιτρέπεται μόνο ρητά εξουσιοδοτημένο `pnputil /add-driver` χωρίς `/install`/`/reboot`, με active-INF και adapter-state guard μετά από κάθε package. Το comparison report μπορεί να εισάγει normalized stage-only manifest, αλλά πρέπει να δηλώνει ότι το current run είναι read-only ενώ το imported experiment έκανε DriverStore mutation. Actual binding χρειάζεται ξεχωριστό connectivity-safe, resumable experiment.
Files affected: `tools\Compare-DriverSources.ps1`, `tools\DriverSourceComparison\SelectionEvidence.ps1`, `tools\DriverSourceComparison\ComparisonReport.ps1`, `internal\Test-DriverSourceComparison.ps1`, `docs\WINDOWS_DRIVER_SOURCE_COMPARISON_CONTRACT.md`, `README.md`, `CHANGELOG.md`, `PROJECT_RULES.md`.
Validation/tests run: Elevated stage-only import of both signed Catalog payloads through direct `gsudo.exe`; confirmed active `oem60.inf`/Wi-Fi `Up` unchanged after each; observed `oem11.inf` `Best Ranked`, `oem60.inf` `Outranked / Installed`, and `oem167.inf` `Outranked`, all rank `00FF0001`; removed only `oem11.inf`/`oem167.inf` without `/uninstall` or `/force`; confirmed baseline `oem60.inf` `Best Ranked / Installed`, adapter `Up`, problem code `0`; parser validation, focused tests, PSScriptAnalyzer 0 Error/Warning, structure guard, LF audit, `git diff --check`, and combined report verdict `CatalogCandidateObservedBestRanked_ActiveUnchanged` passed.

Date: 2026-07-12
Problem: Το stage-only RZ616 experiment απέδειξε Windows ranking αλλά όχι actual activation, ενώ το comparison schema απέρριπτε σωστά κάθε `/install` evidence και δεν μπορούσε να εκφράσει ένα ξεχωριστά εξουσιοδοτημένο non-network activation test. Στο CardReader payload extraction, η πρώτη 7-Zip invocation πέρασε λανθασμένα το `-o` ως χωριστό argument και απέτυχε πριν γράψει payload.
Root cause: Selection proof και activation proof είχαν μοντελοποιηθεί ως ένα μόνο `StageOnlyNoInstall` mode. Επίσης το 7-Zip απαιτεί το output switch και την τιμή του στο ίδιο native argument (`-o<path>`).
Guardrail/rule: Το standalone comparison lab υποστηρίζει δύο ρητά modes: `StageOnlyNoInstall` για risk-sensitive devices και `ControlledActivation` για ξεχωριστά εξουσιοδοτημένα, resumable tests χωρίς automatic reboot. Controlled activation evidence πρέπει να κρατά before/after active INF, status/problem code, Extension state, SetupAPI restart result, loaded binary hash/signature και physical-media test boundary. Μην εξισώνεις healthy loaded driver με verified card/media I/O. Για 7-Zip χρησιμοποίησε πάντα `"-o$output"`, ποτέ χωριστά `-o`, `$output`.
Files affected: `tools\DriverSourceComparison\SelectionEvidence.ps1`, `tools\DriverSourceComparison\ComparisonReport.ps1`, `tools\Compare-DriverSources.ps1`, `internal\Test-DriverSourceComparison.ps1`, `docs\WINDOWS_DRIVER_SOURCE_COMPARISON_CONTRACT.md`, `docs\INSTALLER_ANALYSIS_TOOLKIT.md`, `README.md`, `CHANGELOG.md`, `PROJECT_RULES.md`.
Validation/tests run: Real `DP_CardReader_26040.7z` SHA-256 capture; exact `RtsPerISO.inf` HWID/DriverVer inspection; valid CAT/SYS signatures; elevated stage-only produced `oem11.inf` `Best Ranked`; elevated `/install` activated it without reboot; SetupAPI recorded `Restart verified`; device returned `OK`/problem code `0`; Extension `oem18.inf` remained selected; loaded `RtsPer.sys` version/signature/hash matched payload; zero relevant System warning/error events; Wi-Fi remained `Up`; WUAPI returned zero current target offers; public Catalog returned 25 discovery rows; combined report verdict `CandidateObservedBestRankedAndActivated`; parser, focused tests, PSScriptAnalyzer 0 Error/Warning, structure guard, LF audit, live health assertion, and `git diff --check` passed.

Date: 2026-07-12
Problem: Το standalone driver lab παράγει ισχυρό evidence, αλλά η τρέχουσα console summary είναι πολύ απλή για καθημερινή χρήση και το recommendation algorithm χρειάζεται επαναλαμβανόμενα πραγματικά cases πριν θεωρηθεί κατάλληλο για ενσωμάτωση στο main DeviceCheck tree.
Root cause: Η πρώτη φάση έδωσε προτεραιότητα σε extraction, tracing, ranking και normalized reports. Δεν υπάρχει ακόμη product-quality Windows Terminal presentation ούτε regression corpus που να αποδεικνύει ότι οι recommendation rules γενικεύονται πέρα από τα λίγα tested devices.
Guardrail/rule: Πριν από main-TUI integration, το standalone lab πρέπει να έχει width-aware, colored Windows Terminal UI με χρήσιμα icons/emojis, καθαρό separation Evidence / Interpretation / Recommendation / Risk / Actions και πλήρες “why this option” explanation. Κάθε πραγματικό driver test πρέπει να αποθηκεύεται ως normalized case και regression fixture· το algorithm βελτιώνεται μέσω explicit verified rules και replayable cases, όχι με opaque ή αυτόματο “learning”. Χρησιμοποίησε πρώτα αυτό το test PC ως controlled corpus και μην γενικεύεις ακόμη σε άλλα home/work PCs. Κράτα το workflow πρακτικό ώστε αργότερα να μπορεί να χρησιμοποιηθεί σε πολλά machines χωρίς να απαιτεί υπερβολικό χρόνο ανά device.
Files affected: `PROJECT_RULES.md`.
Validation/tests run: Documentation-only product-direction decision; implementation και UI validation παραμένουν επόμενο gate.

Date: 2026-07-12
Problem: Το standalone lab είχε ισχυρό JSON/Markdown evidence αλλά όχι πρακτικό WT presentation, explainable recommendation layer, reusable verified cases ή αυτόματη μετάβαση από EXE trace σε per-device source comparison. Preview-only reruns επίσης αγνοούσαν παλιότερο completed evidence για το ίδιο installer.
Root cause: Το initial implementation επικεντρώθηκε στο evidence collection. Windows verdict, user recommendation, activation risk και next action δεν ήταν ξεχωριστά contracts, ενώ το trace adapter απέδιδε outcome από οποιοδήποτε SetupAPI node αντί να ταυτοποιεί package nodes με INF/date/version.
Guardrail/rule: Κράτα τον Advisor standalone από το main TUI. Render με width-aware synchronized WT frames και χωριστά `Overview/Sources/Candidates/Evidence/Actions`. Recommendation objects πρέπει να έχουν stable code, confidence, evidence level, activation risk, reasons, caveats και non-executing action metadata. Attribution σε SetupAPI package node απαιτεί original INF + DriverVer date + normalized version. Preview-only trace μπορεί να επαναχρησιμοποιεί completed sibling trace μόνο με ίδιο installer SHA-256 και target Instance ID. Κάθε verified machine outcome γίνεται replayable regression case· rules δεν αλλάζουν χωρίς να περνούν όλα τα cases.
Files affected: `tools\DriverAdvisor\*.ps1`, `tools\Invoke-DriverPackageAdvisor.ps1`, `tools\Invoke-DriverTraceAdvisor.ps1`, `tools\Compare-DriverSources.ps1`, `tools\DriverSourceComparison\TraceEvidence.ps1`, `tools\DriverSourceComparison\ComparisonReport.ps1`, `tools\Trace-DriverPackageImpact.ps1`, `tools\Launch-DriverPackageImpactTrace.vbs`, `internal\Test-DriverAdvisor.ps1`, `internal\Test-DriverSourceComparison.ps1`, `internal\fixtures\driver-advisor\*.case.json`, `docs\DRIVER_PACKAGE_ADVISOR.md`, `README.md`, `CHANGELOG.md`, `PROJECT_RULES.md`.
Validation/tests run: Six Driver Advisor regression cases; plain/colored width tests at 60/80/120 columns across five views; exact 24-row frame assertions; parser and focused comparison tests; real local-only CardReader report render; real Lenovo WLAN completed-trace render; real preview-only trace render; historical trace SHA/Instance reuse; real Smart Appearance multi-device replay with per-instance `ObservedAppliedExtension` only for Integrated Camera; real Realtek Audio/Dolby selected-component replay; real WLAN EXE `PreviewOnly + LaunchAdvisor` full WU/Catalog pipeline; PSScriptAnalyzer 0 Error/Warning στα νέα/modular files; DeviceCheck structure guard; six JSON fixture parses; LF source/docs/json και CRLF VBS verification; VBS `-LaunchAdvisor` wiring assertion; `git diff --check`; main `DeviceCheck.ps1` unchanged assertion; live CardReader `oem11.inf`/Wi-Fi `Up` health guard.

Date: 2026-07-12
Problem: Το single `.exe` context-menu verb δεν επέτρεπε γρήγορη επιλογή μεταξύ safe preview, πραγματικού install trace και Advisor output. Η απευθείας έκθεση των τεσσάρων standalone scripts θα δημιουργούσε broken entries, επειδή μόνο ο trace coordinator δέχεται installer `.exe`.
Root cause: `Invoke-DriverPackageAdvisor.ps1` απαιτεί comparison report, `Invoke-DriverTraceAdvisor.ps1` απαιτεί trace folder και `Compare-DriverSources.ps1` απαιτεί device identity. Αυτά πρέπει να καλούνται μετά το EXE matching, όχι απευθείας με `%1`.
Guardrail/rule: Το `.exe` Explorer integration παραμένει κάτω από `HKCU\Software\Classes\SystemFileAssociations\.exe\shell`. Στο τρέχον Windows 11 Explorer build, inline `SubCommands=""` με children κάτω από το ίδιο `SystemFileAssociations` parent εμφάνισε arrow αλλά παρέμεινε inert. Χρησιμοποίησε `ExtendedSubCommandsKey=DeviceCheck.DriverPackageTools` και κράτα τα nested children κάτω από το ιδιωτικό `HKCU\Software\Classes\DeviceCheck.DriverPackageTools\shell` tree. Τα τέσσερα EXE-aware modes είναι `PreviewAdvisor`, `TraceAdvisor`, `PreviewOnly`, `TraceOnly`. Preview modes περνούν ρητά `-PreviewOnly`, install modes `-RunInstaller`, Advisor modes `-LaunchAdvisor`, και ο launcher απορρίπτει άγνωστα modes. Μην επαναφέρεις optional verbs κάτω από `exefile\shell`.
Files affected: `Install-DriverPackageTraceContextMenu.ps1`, `tools\Launch-DriverPackageImpactTrace.vbs`, `README.md`, `docs\DRIVER_PACKAGE_ADVISOR.md`, `CHANGELOG.md`, `PROJECT_RULES.md`.
Validation/tests run: Installer parser validation; case-insensitive four-mode VBS wiring assertion; CRLF VBS verification; live HKCU install; `reg.exe query /s` readback confirmed four nested commands and exact mode arguments; Computer Use visual check first caught the missing arrow without `SubCommands=""`, then confirmed the corrected cascade and all four visible children; legacy `SystemFileAssociations` single verb and unsafe `exefile\shell` verb both absent; `git diff --check`; no Explorer restart.

Date: 2026-07-12
Problem: Το interactive Driver Package Advisor φαινόταν σωστό σε static snapshots, αλλά κατά την εναλλαγή `Overview/Sources/Candidates/Evidence/Actions` άφηνε fragments και horizontal rules από το προηγούμενο view πάνω στις μικρότερες νέες γραμμές.
Root cause: Το renderer έκανε cursor-home και τελικό `ESC[J`, όμως οι ενδιάμεσες rows δεν έκαναν erase το υπόλοιπο της παλιάς row. Το newline μετακινούσε τον cursor χωρίς να καθαρίζει stale suffixes, ενώ το τελικό `ESC[J` καθάριζε μόνο από την τελευταία cursor position και κάτω.
Guardrail/rule: Κάθε row ενός immediate-mode interactive WT frame πρέπει να τελειώνει με `ESC[K` πριν το newline, εκτός αν αποδεδειγμένα γεμίζει ολόκληρο το viewport width. Κράτα το row-erasure conversion κοινό για Advisor και selectors. Static line-count/width tests δεν υποκαθιστούν screenshot validation μετά από εναλλαγή μεταξύ views με διαφορετικά line lengths.
Files affected: `tools\DriverAdvisor\TerminalRenderer.ps1`, `internal\Test-DriverAdvisor.ps1`, `CHANGELOG.md`, `PROJECT_RULES.md`.
Validation/tests run: Parser validation; six-case Driver Advisor regression suite; 60/80/120 width and exact 24-row checks; every interactive row asserted to end with `ESC[K`; PSScriptAnalyzer 0 Error/Warning; `pyte 0.8.2` plus `wcwidth 0.8.2` installed under ignored `.devicecheck-data\test-tools` after official PyPI metadata/license/hash verification; isolated 120×30 VT replay of `Overview → Sources` proved zero stale suffixes across all 30 rows; DeviceCheck structure guard; `git diff --check`. User screenshots supplied the real Windows Terminal reproduction because Computer Use cannot target terminal apps.

Date: 2026-07-12
Problem: Το standalone Advisor απαιτούσε manual `-SdioReportPath`, εμφάνιζε `SDIO: Δεν δόθηκε` και `Selection: Δεν δόθηκε`, και εξαρτιόταν πρακτικά από τα δυσνόητα SDIO GUI filters. Το matcher log επίσης κόβει μεγάλα pack names από αριστερά και μπορεί να περιέχει index rows των οποίων το `.7z` payload δεν υπάρχει στο active `drivers` root. Ένα δεύτερο headless SDIO instance κόλλησε όταν υπήρχε ήδη ανοιχτό SDIO process.
Root cause: Δεν υπήρχε target-to-local-SDIO orchestration, cache freshness rule, payload-availability check ή explicit linkage μεταξύ active driver, extracted OEM INF και SDIO candidates. Το υπάρχον `Invoke-SdioDriverAudit.ps1` είχε ασφαλές `-disableinstall` mode αλλά indefinite `Start-Process -Wait`.
Guardrail/rule: Το standalone trace Advisor κάνει automatic SDIO audit, όχι GUI automation. Προτίμησε complete matcher log που είναι νεότερο από τα active packs· αλλιώς τρέξε `-disableinstall -nogui -preservecfg` με bounded timeout. Αν τρέχει ήδη SDIO και δεν υπάρχει current log, επέστρεψε deferred outcome αντί για δεύτερο instance. Resolve truncated pack names μόνο με μοναδικό suffix match και μέτρα ως available μόνο candidates με πραγματικό `.7z` payload. Κράτα index-only rows χωριστά, deduplicate με INF CRC/version/matched ID, και μην μετατρέπεις SDIO score σε Windows rank. Αν OEM και active έχουν ίδιο matched ID/date/version, χρησιμοποίησε `NotRequiredSameAsActive` και μην προτείνεις selection test.
Files affected: `internal\Invoke-SdioDriverAudit.ps1`, `tools\DriverSourceComparison\SdioLocalEvidence.ps1`, `tools\Compare-DriverSources.ps1`, `tools\Invoke-DriverTraceAdvisor.ps1`, `tools\DriverSourceComparison\ComparisonReport.ps1`, `tools\DriverAdvisor\RecommendationEngine.ps1`, `tools\DriverAdvisor\TerminalRenderer.ps1`, `internal\Test-DriverAdvisor.ps1`, `internal\Test-DriverSourceComparison.ps1`, `internal\fixtures\driver-advisor\rz616-local-sdio-linked.case.json`, `README.md`, `docs\DRIVER_PACKAGE_ADVISOR.md`, `docs\WINDOWS_DRIVER_SOURCE_COMPARISON_CONTRACT.md`, `CHANGELOG.md`, `PROJECT_RULES.md`.
Validation/tests run: Real `E:\SDIO` RZ616 Bluetooth audit matched 1/251 devices; exact `USB\VID_0489&PID_E0D8&MI_00`; real pack recovery found `DP_Bluetooth_SDIO01_26072.7z`; active/OEM `26.40.2.228` linked as identical; six available unique SDIO candidates were older and zero were newer. A timed-out second SDIO instance was stopped without touching the pre-existing SDIO process or Explorer. Final auto-detection found `E:\SDIO`, reused `ExistingLog`, and created no SDIO process. Seven Advisor replay cases; 60/80/120 width checks; focused comparison tests; parser validation across 20 touched/new PowerShell files; PSScriptAnalyzer 0 warnings/errors across nine focused files; DeviceCheck structure guard; seven JSON fixture parses; LF/CRLF policy; changed/untracked trailing-whitespace scan; `git diff --check`; and main `DeviceCheck.ps1` unchanged assertion all passed.

Date: 2026-07-12
Problem: Το RZ616 Wi-Fi Advisor εμφάνισε `NoMatchingCandidateInLocalDriverpacks` παρότι το flash drive είχε όλα τα current SDIO packs στο `E:\SDIO\drivers`.
Root cause: Το complete matcher log διατηρούσε candidate identities από τα παλιά `_P_SDIO01_26044.bin` και `_P_WLAN-WiFi_26040.bin`, ενώ τα πραγματικά payloads είχαν μετακινηθεί στα current `DP_SDIO01_26072.7z` και `DP_WLAN-WiFi_26061.7z`. Ο ασφαλής resolver απαιτούσε exact/truncated suffix pack-name match και σωστά δεν υπέθετε ότι ένα διαφορετικό generation περιέχει το ίδιο INF. Ωστόσο, το current WiFi archive πράγματι περιείχε exact RZ616 `mtkwl6ex.inf` payloads.
Guardrail/rule: Όταν ένα SDIO index row δεν επιλύεται σε exact pack suffix, επιτρέπεται read-only archive fallback μόνο σε current `.7z` packs με κοινό semantic family token. Μην εμπιστεύεσαι μόνο pack generation, filename ή INF basename: εξήγαγε το candidate INF σε stdout με verified 7-Zip και απαίτησε exact Hardware ID και ίδιο `DriverVer` date/version. Deduplicate unresolved index rows μετά το archive resolution, ώστε ένα old-index duplicate ενός verified current payload να μην μετρά ως index-only.
Files affected: `tools\DriverSourceComparison\SdioLocalEvidence.ps1`, `README.md`, `docs\DRIVER_PACKAGE_ADVISOR.md`, `docs\WINDOWS_DRIVER_SOURCE_COMPARISON_CONTRACT.md`, `CHANGELOG.md`, `PROJECT_RULES.md`.
Validation/tests run: 7-Zip 26.02 inspection of `E:\SDIO\drivers\DP_WLAN-WiFi_26061.7z` found 12 `mtkwl6ex.inf` files. Exact archive verification linked four available RZ616 candidates: `25.40.2.586` (`SameAsActive`, `SAME+CURRENT+DUP`), `3.5.0.1376`, `0.34.2.886`, and `3.3.0.789`; zero were newer. Only the absent exact `3.5.0.1380` remained index-only. Full WUAPI/OEM/SDIO Advisor regeneration returned `NoCurrentWuOfferObserved`, `ObservedOutrankedCandidate`, and `KeepCurrent_NoVerifiedBetterCandidate`.

Date: 2026-07-12
Problem: Ακόμη και package με δύο INF χρειαζόταν περίπου 15 seconds για preview και Advisor, ενώ το ίδιο design θα επαναλάμβανε ακριβό SDIO parsing ανά target device σε μεγάλο package όπως AMD Chipset.
Root cause: Το explicit `PreviewOnly` συνέλεγε full before snapshot ολόκληρου του PC (`4.803s`) παρότι δεν μπορούσε να αλλάξει state. Το comparison έκανε οκτώ ξεχωριστά `Get-PnpDeviceProperty` calls και πάντα `Win32_PnPSignedDriver` (`6.425s` local evidence). Το SDIO helper έκανε parse το ίδιο 251-device matcher log για κάθε target και επιθεωρούσε current/older candidates που δεν ενδιέφεραν το fast update question. WUAPI/Catalog δεν είχαν ενιαίο explicit opt-in contract σε όλα τα entry points.
Guardrail/rule: Default standalone flow είναι local/offline και μετρήσιμο. WUAPI/Catalog τρέχουν μόνο με `-IncludeWindowsUpdate`/`-IncludeCatalog`. SDIO policy είναι `Newer + Better match + Show only best`: κράτα μόνο πρώτο candidate με `NEW` ή `BETTER`, απόκλεισε current/old/worse/dup/invalid/missing πριν από archive inspection. Parse το full SDIO log μία φορά σε cache και κάνε per-device lookup. Χρησιμοποίησε bulk PnP property read και κάνε signer CIM lookup μόνο με explicit signature-evidence switch. Full before snapshot απαιτείται για install trace, όχι για explicit preview-only. Κάθε major phase πρέπει να γράφει benchmark timing σε JSON.
Files affected: `tools\DriverSourceComparison\Benchmark.ps1`, `tools\DriverSourceComparison\LocalEvidence.ps1`, `tools\DriverSourceComparison\SdioLocalEvidence.ps1`, `tools\Compare-DriverSources.ps1`, `tools\Invoke-DriverTraceAdvisor.ps1`, `tools\Trace-DriverPackageImpact.ps1`, `tools\DriverSourceComparison\ComparisonReport.ps1`, `tools\DriverAdvisor\RecommendationEngine.ps1`, `tools\DriverAdvisor\TerminalRenderer.ps1`, `internal\Test-DriverSourceComparison.ps1`, `README.md`, `docs\DRIVER_PACKAGE_ADVISOR.md`, `docs\WINDOWS_DRIVER_SOURCE_COMPARISON_CONTRACT.md`, `CHANGELOG.md`, `PROJECT_RULES.md`.
Validation/tests run: Real WLAN baseline comparison measured `8.736s`: local Windows evidence `6.425s`, SDIO `2.029s`. Fine-grained probes found `Win32_PnPSignedDriver 2.142s`, eight PnP property calls about `3.15s`, and SDIO matcher parse `1.838–2.604s`. Bulk PnP plus optional signer reduced local evidence to `0.77–1.36s`. Full-audit cache reduced repeat SDIO lookup to `0.214–0.608s`; repeat comparison reached `1.011s`. WLAN preview dropped from `6.727s` to `1.878s` by skipping full inventory only in PreviewOnly. Integrated IR Camera verified one eligible `SAME+NEW` candidate (`5.0.18.305`) with `ShowOnlyBest`, while RZ616 Wi-Fi correctly returned `NoNewerOrBetterCandidate`. Final context-equivalent WLAN run proved WUAPI/Catalog disabled by default, automatic `E:\SDIO` detection, all three benchmark files present, and `KeepCurrent_NoVerifiedBetterCandidate`. Parser validation across 21 PowerShell files; focused PSScriptAnalyzer 0 warnings/errors; trace script 0 analyzer errors; seven Advisor cases; SDIO filter/benchmark fixtures; source-comparison tests; structure guard; whitespace/line-ending/diff checks; and main `DeviceCheck.ps1` unchanged assertion passed.

Date: 2026-07-12
Problem: Ένα OEM installer όπως το `Realtek Audio Driver.exe` εμφανιζόταν ως flat list 13 INF/device rows, χωρίς να φαίνεται ότι αφορά δύο stack roots, Extension layers και έξι linked software/APO components. Τα `ComponentIDs` ενός Extension INF μετριούνταν επίσης σαν δεύτερο direct driver match πάνω στο child node.
Root cause: Το preview έκανε raw INF text matching και το multi-device Advisor χρησιμοποιούσε απλό selector. Δεν υπήρχε role-aware topology contract μεταξύ extraction και UI ούτε διάκριση `FunctionBinding`, `ExtensionApplication`, `ComponentDeclaration` και `ComponentBinding`.
Guardrail/rule: Κάθε standalone trace γράφει `package-topology.json`. Μετρά potential package applications μόνο από direct function/component bindings και Extension applications· κρατά declaration rows ως relationship evidence, όχι ως δεύτερους drivers. Πριν από per-device Advisor, εμφάνισε standalone DeviceCheck-style filtered tree μόνο με package-related present nodes, stack-root/component markers και scrollable details. Κράτα το Package Device View και όλο το extraction/topology logic έξω από το main `DeviceCheck.ps1` μέχρι explicit integration request.
Files affected: `tools\DriverPackageView\Topology.ps1`, `tools\DriverPackageView\TerminalRenderer.ps1`, `tools\Show-DriverPackageView.ps1`, `tools\Trace-DriverPackageImpact.ps1`, `tools\Invoke-DriverTraceAdvisor.ps1`, `internal\Test-DriverPackageView.ps1`, `README.md`, `docs\DRIVER_PACKAGE_ADVISOR.md`, `CHANGELOG.md`, `PROJECT_RULES.md`.
Validation/tests run: Real `Realtek Audio Driver.exe` PreviewOnly trace produced 11 INF, 8 present nodes, 11 potential applications, 7 direct bindings, 4 Extension applications, 2 stack roots and 6 linked component edges; the two Dolby declaration rows no longer count as independent driver applications. Synthetic base/Extension/component fixture; 80/104/105/140/200-width and interactive row-erasure checks; topology replay across 25 recent previews including no-match packages; no-admin trace-to-topology-to-report run; completed-trace per-device Advisor continuation; regenerated Realtek Audio, WLAN and AMD Chipset traces; Driver Package View, Advisor and source-comparison tests; parser validation; focused PSScriptAnalyzer 0 warnings/errors; trace script 0 analyzer errors; DeviceCheck structure guard; trailing-whitespace and `git diff --check`; main `DeviceCheck.ps1` unchanged assertion.

Date: 2026-07-15
Problem: Στο Offline Snapshot Library, κάθε Up/Down redraw άφηνε διπλό `Offline Snapshots: <network>` και παλιές rows στο Windows Terminal.
Root cause: Το line-aware item viewport χρησιμοποιούσε `WindowHeight - 10`, αλλά το frame έχει 11 fixed physical lines και κάθε `Add-UiFrameLine` τελειώνει με newline. Έτσι το frame έγραφε δύο rows πέρα από το ασφαλές `WindowHeight - 1`, προκαλώντας terminal scroll πριν από το επόμενο cursor-home redraw.
Guardrail/rule: Για fullscreen immediate-mode frames, το visible-content budget πρέπει να είναι `WindowHeight - 1 - fixedFrameLines`. Μέτρα όλες τις banner/spacer/indicator/footer rows και κράτα μία τελική row αχρησιμοποίητη όταν κάθε frame line περιλαμβάνει newline. Τα line-aware item counts δεν αντικαθιστούν το συνολικό physical-frame budget. Κάλυψε το αποτέλεσμα με VT replay πολλών navigation frames και assert για zero scrolls/duplicate headers.
PowerShell validation guardrail: Το `Invoke-ScriptAnalyzer -Path` δέχεται scalar `string`, όχι array από paths. Για focused multi-file analysis, κάνε `foreach` ανά αρχείο και συγκέντρωσε τα findings· μην περνάς comma-separated path array απευθείας στο `-Path`.
Native-command validation guardrail: Μην γράφεις `if (git diff --quiet ...)` στο PowerShell, επειδή ένα silent native command δεν παράγει pipeline value και μπορεί να αξιολογηθεί ως `$false` ακόμη με exit `0`. Εκτέλεσε το command και έλεγξε αμέσως `$LASTEXITCODE` (`0` clean, `1` diff, άλλο error).
Files affected: `internal\DeviceCheck\06-RemoteConnectionOfflineMenu.ps1`, `internal\Test-DeviceCheckOfflineMenu.ps1`, `CHANGELOG.md`, `PROJECT_RULES.md`.
Validation/tests run: PowerShell parser validation; focused viewport-budget assertions at 22/30/44 rows; `pyte 0.8.2` 156x44 terminal / 154-column UI replay of two navigation frames with zero viewport scrolls and zero duplicate banner/content headers; PSScriptAnalyzer 1.25.0 differential check (new test 0 findings, production file unchanged at its 12 pre-existing warnings); DeviceCheck structure guard; LF policy; `git diff --check`; main `DeviceCheck.ps1` unchanged guard.
