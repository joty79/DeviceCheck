...
- Added `docs\INSTALLER_ANALYSIS_TOOLKIT.md` with the installed 7-Zip, innoextract, Detect It Easy, lessmsi, WiX, Universal Extractor 2, Strings, Procmon, and Windows Installer workflows. The guide records a real AMD Chipset Software `8.05.04.516` comparison where InstallShield `/a` produced 414 files and 68 INF files without changing DriverStore/SetupAPI state, while generic extractors were incomplete.
- Tightened package-preview matching by rejecting bare generic USB root-hub hardware IDs such as `USB\ROOT_HUB30`. AMD-specific `USB\ROOT_HUB30&VID1022&PID...` matches remain valid, removing three false preview matches from the extracted AMD chipset package.
- Improved no-INF package preview for nested driver installers such as AMD Chipset Software. Extracted setup/driver executables are now classified as `Nested installer/bootstrapper payload`, executable version metadata is retained, and reports explain that final INF files may appear only while the child installer runs, making before/after and SetupAPI evidence authoritative.
- Added a read-only post-reboot mode to the standalone driver package tracer through `-PostRebootTraceDirectory`. It records current matched-device health, active function drivers, installed Extension/configuration drivers, trace-added package state, and directly relevant post-boot warning/error events in `post-reboot-report.md` plus raw JSON evidence.
- Added `tools\Get-DriverPackageTraceSummary.ps1` to consolidate the latest result per installer and classify every trace-added published INF as an active function driver, installed Extension/configuration, stored-only, or missing from the Driver Store. The summary is explicitly evidence rather than a recommendation or cleanup list.
- Added persisted and reported installer exit-code interpretation for success, ambiguous wrapper/child-process code `259`, user cancellation, restart initiated (`1641`), restart required (`3010`), and other non-zero results.
- Added structured SetupAPI applied-device-configuration evidence to driver trace reports. Reports now detect newly staged INFs used in `Install Device: Configuration` entries, device subtree removal/restart, and PnP install results, so an applied Extension INF is no longer mislabeled as staging-only when the visible function-driver binding stays unchanged. SetupAPI device IDs containing embedded `{GUID}` values are now captured through the final log delimiter instead of being truncated at the first brace.
- Replaced the package-preview `Format-Table -AutoSize -Wrap` output with width-aware candidate blocks. Long hardware IDs and extracted INF paths now use the available terminal width with aligned continuation lines instead of forcing the INF column down to a few characters and creating hundreds of mostly blank console lines.
- Improved zero-match driver package reports by extracting and listing supported hardware IDs from each INF, so staged/no-present-device packages show exactly which hardware variants they target. INF metadata now resolves `%Token%` values through `[Strings]`, producing provider names such as `Egis Technology Inc.` and `Goodix` instead of raw placeholders.
- Improved no-change driver trace verdicts by comparing an exact matched package INF with the active driver from the same original INF family. Reports now show package versus active date/version as `Older`, `Same`, or `Newer`, and future installer runs persist `run-metadata.json` with installer start/end time, exit code, and after-snapshot capture time.
- Fixed SetupAPI driver-stack parsing in `tools\Trace-DriverPackageImpact.ps1` so the final `Driver Node` is closed at selection/update/section boundaries instead of being overwritten by metadata from a later package import. The stack table now includes the device instance ID, retains all filtered SetupAPI evidence instead of truncating it at 500 lines, and active-driver diffs/verdicts now include added and removed signed-driver bindings as well as updates to existing bindings.
- Improved `tools\Trace-DriverPackageImpact.ps1` package preview matching by ignoring overly broad compatible IDs such as vendor-only `PCI\VEN_####`, class-only `PCI\CC_*`, generic USB class IDs, generic HDAUDIO function IDs, and the bare camera group ID `SensorGroup`. Regenerated trace reports now rebuild `package-preview.json` from the original installer when available, so old trace folders pick up newer matching rules.
- Improved staged-package verdicts in `tools\Trace-DriverPackageImpact.ps1` reports. Packages imported by SetupAPI with `Unable to find any matching devices` are now labeled `Staged only; no matching present device`, and all-staged/no-match runs get a clearer summary verdict.
- De-duplicated `SetupAPI Selected Driver Stack` rows in `tools\Trace-DriverPackageImpact.ps1` reports, so repeated SetupAPI sections for the same selected/outranked driver nodes no longer make reports look noisier than the actual driver decision.
- Moved the `.exe` driver trace context menu from the unsafe per-user `exefile\shell` branch to `SystemFileAssociations\.exe\shell`, and added cleanup for the legacy key so double-clicking `.exe` files keeps using the normal Windows `open` verb.
- Improved `tools\Trace-DriverPackageImpact.ps1` reports for MSI-based OEM payloads. No-INF packages now include payload kind, non-INF payload file summaries, and MSI metadata such as product name, version, and manufacturer, so provisioning/application packages like Dolby Vision are not mislabeled as cleanup utilities.
- Improved `tools\Trace-DriverPackageImpact.ps1` reports for non-driver Lenovo utility packages. Existing extracted payload folders are now shown even when they contain no INF files, payload file count is reported, and no-INF/no-driver-change runs are labeled as utility/cleanup packages rather than looking like failed driver traces.
- Improved `tools\Trace-DriverPackageImpact.ps1` reports for Extension INF packages. Reports now include a `SetupAPI Selected Driver Stack` table showing selected/outranked driver nodes, selected Extension INF nodes, original INF names, provider/class, date/version, rank, Extension ID, and configuration; `-RegenerateTraceDirectory` can rebuild a report/diff from an existing trace folder without rerunning the installer.
- Added `SetupAPI Outcome Signals` to `tools\Trace-DriverPackageImpact.ps1` reports. Empty diffs now still surface installer activity such as `Already Imported`, `Device does not need an update`, `No better matching drivers found`, `No devices were updated`, and SetupAPI section exit status, making already-installed packages easier to interpret.
- Added driver dates to `tools\Trace-DriverPackageImpact.ps1` human reports. The active-driver and staged-package tables now split date/version and label cases where the same version is already active with a newer driver date, which explains Wacom-style outcomes where Windows stages Lenovo's package but keeps the newer active binding.
- Improved `tools\Trace-DriverPackageImpact.ps1` reports so staged DriverStore packages are separated from active device-driver changes. Reports now show a verdict, current active drivers for matched devices, staged package impact, and same-version already-active evidence such as `oem145.inf` staged while `oem19.inf` remains active for the same `rtsper.inf` version.
...
- Grouped each Offline Snapshots network view into always-expanded `Laptops`, `Desktops`, and `Unknown` sections. The classifier uses captured ACPI battery devices first, then laptop/desktop model keywords and desktop CPU suffixes, so existing snapshots can be organized without recapturing them.
- Added stronger laptop/desktop evidence to new local and remote snapshots by capturing `Win32_ComputerSystem.PCSystemType`, `PCSystemTypeEx`, `Win32_SystemEnclosure.ChassisTypes`, and `Win32_Battery`, and updated the Offline Snapshots classifier to prefer chassis/system-type evidence before battery, model, or CPU heuristics.
- Added a colored `Type` row near the top of Computer Info, above the hardware label, so local, remote, and offline snapshots show `Laptop`, `Desktop`, or `Unknown` with the classifier confidence/reason.
- Moved the remote/offline snapshot file path out of the top Computer Info rows into a separated bottom `Snapshot Info` section, keeping the rarely used path visible without interrupting the main hardware identity fields.
- Backfilled existing offline snapshots with `Collector.DeviceKind*` metadata and index fields, so old `DESKTOP-*` snapshots can be grouped and identified as laptops/desktops without being recaptured.
...
- Modified `Invoke-ConnectLanTarget` in [06-RemoteConnection.ps1](file:///d:/Users/joty79/scripts/DeviceCheck/internal/DeviceCheck/06-RemoteConnection.ps1) so that when canceling the connection selection menu via the `Escape` key, it automatically resets the target mode back to the local host machine, performs a fresh local system scan, and refreshes the TUI view, rather than staying on the previously loaded remote snapshot target.
- Moved LAN discovery filtering and hosts-cache helpers into [06-RemoteDiscoveryFilters.ps1](file:///d:/Users/joty79/scripts/DeviceCheck/internal/DeviceCheck/06-RemoteDiscoveryFilters.ps1), keeping [06-RemoteConnection.ps1](file:///d:/Users/joty79/scripts/DeviceCheck/internal/DeviceCheck/06-RemoteConnection.ps1) below the structural budget while making discovery filtering reusable.
...
- Optimized the `Ctrl+L` connection selector menu and the offline snapshots submenu performance in [06-RemoteConnection.ps1](file:///d:/Users/joty79/scripts/DeviceCheck/internal/DeviceCheck/06-RemoteConnection.ps1) and [06-RemoteConnectionOfflineMenu.ps1](file:///d:/Users/joty79/scripts/DeviceCheck/internal/DeviceCheck/06-RemoteConnectionOfflineMenu.ps1) by caching the connection history, offline snapshots, and network scan logs outside of the interactive TUI event loop, preventing slow disk scans and file reads on every key press. Added high-precision timing measurements and logged them to the TUI benchmark log for improved diagnostic visibility.
- Optimized the Explorer Network computer seed used by `Ctrl+L` discovery by replacing the helper `pwsh` child process with a bounded STA runspace, keeping the same timeout safety while avoiding process startup overhead during refresh.
- Optimized monitor INF driver friendly name matching in [internal\MonitorEdidResolver.psm1](file:///d:/Users/joty79/scripts/DeviceCheck/internal/MonitorEdidResolver.psm1) by replacing the slow, line-by-line PowerShell recursive text scan of all `oem*.inf` files (which took ~2.8 seconds and caused UI lag when selecting monitor devices) with a compiled C#-native `Select-String` search (~70ms), and automatically bypassed the search on remote snapshot/offline targets where the host system's INF folder does not apply.
- Optimized monitor WMI/CIM retrieval in [internal\MonitorEdidResolver.psm1](file:///d:/Users/joty79/scripts/DeviceCheck/internal/MonitorEdidResolver.psm1) by querying classes in bulk once and caching them in module scope (`$script:GlobalWmiMonitorIDs`, etc.), pre-warming this WMI cache asynchronously/synchronously at startup and rescan to avoid first-time selection render lags, caching hardware ID breakdown lines in `$script:HardwareIdBreakdownCache` in [04-UiTextFormatting.ps1](file:///d:/Users/joty79/scripts/DeviceCheck/internal/DeviceCheck/04-UiTextFormatting.ps1), clearing all query/resolver caches during rescan/cache invalidation in [03-EvidenceResolvers.ps1](file:///d:/Users/joty79/scripts/DeviceCheck/internal/DeviceCheck/03-EvidenceResolvers.ps1), and adding immediate remote target bypasses, reducing TUI device selection render latency under 50ms.


### Added
- Added `tools\Trace-DriverPackageImpact.ps1`, `tools\Launch-DriverPackageImpactTrace.vbs`, and `Install-DriverPackageTraceContextMenu.ps1` as a standalone driver installer impact lab. The `.exe` context menu launches elevated, previews matching extracted INFs, captures before/after DriverStore, `C:\Windows\INF`, PnP, signed-driver, system-driver, and SetupAPI evidence, then writes a readable `report.md` plus raw JSON snapshots under `.devicecheck-data\driver-package-traces`.
- Added `docs\DRIVER_EVIDENCE_DECISION_MODEL.md`, capturing the DeviceCheck product direction as a driver evidence explorer, source-comparison model, risk-label system, and future before/after installer-change tracker.
- Added a current-user Windows Explorer context menu installer (`Install-DeviceCheckContextMenu.ps1`) plus `Launch-DeviceCheck.vbs`, exposing `DeviceCheck` on desktop/folder background, folders, and drives with the bundled `assets\devicemanager.ico`; the launcher now starts DeviceCheck elevated so hardware/registry evidence paths have the expected access.
...
- Added `internal\Test-RemoteDiscoveryLive.ps1` as a repeatable live LAN smoke for `Ctrl+L` discovery parity checks, with optional expected host/IP assertions, `-CompareExplorer` for comparing DeviceCheck results against Windows Explorer's `Network > Computer` view, `-RequireExplorerRows` for strict shop parity runs, and `-RepeatCount` / `-RepeatDelaySeconds` for catching intermittent or delayed discovery.
- Added ICMPv4 Echo Request firewall auditing/fixing to `Diagnose-SmbSharing.ps1` so the same network visibility helper can repair ping responses in addition to SMB and Network Discovery.
...
### Fixed
- Fixed remote snapshot failures with reachable WinRM targets but rejected credentials. The connection error screen now labels credential rejection separately from offline/firewall failures, shows the attempted user, explains blank-password local-account entry, and offers `R` to retry credentials immediately instead of forcing the user back to the selector.
- Fixed `Ctrl+L` LAN discovery for SMB/RDP/WinRM-ready PCs that are not yet in neighbor/history and do not answer the initial WS-Discovery probe. Discovery now includes a short-budget subnet sweep for computer ports `445`, `3389`, and `5985`, which catches ready PCs before they are manually contacted through Explorer, Remote Desktop, or WinRM without adding the slower ping-gated sweep cost.
- Optimized `Ctrl+L` PC discovery after the subnet sweep by ignoring tentative APIPA adapter prefixes, batching the PC-port sweep across active LAN prefixes, skipping foreground ARP purges, and removing unused ICMP ping work from the PC-only selector path.
- Fixed the `Ctrl+L` discovered PC list so ARP/ping-only LAN devices are no longer shown as PCs. Closed-management yellow rows now require a WS-Discovery `pub:Computer` signal and render as `(Computer - mgmt closed)`.
- Fixed `Ctrl+L` LAN discovery delays and IP-only rows caused by stale `IP -> IP` host cache entries. IP-only cache/history values now remain unresolved and are retried through active name resolution instead of blocking later name discovery.
- Fixed `Ctrl+L` LAN discovery noise after adding detected-only hosts by filtering multicast/reserved neighbor entries such as `224.*`, `239.*`, `mdns`, and `igmp`, and by keeping IP-only reverse DNS names as the full IP instead of truncating them to `192`.
- Fixed `Ctrl+L` LAN discovery so ARP/ping-visible hosts with both WinRM `5985` and SMB `445` closed are still shown as `(Detected - mgmt closed)` instead of disappearing from the selector. This makes newly connected PCs visible before WinRM is enabled while still making clear that snapshot collection needs WinRM.
...
- Fixed DHCP IP address conflicts between home and work networks when devices share the same IP (e.g., `192.168.1.7` assigned to a work PC and statically mapped to `PALIOS` at home) in [06-RemoteConnection.ps1](file:///d:/Users/joty79/scripts/DeviceCheck/internal/DeviceCheck/06-RemoteConnection.ps1):
  ...
  - Implemented MAC address and Hostname mismatch verification guards in `Test-DeviceCheckHistoryEntryOnline` and `Invoke-ConnectionHistorySelector` to prevent incorrect active/online mappings when different devices share the same IP.
  - Added MAC address verification in `Resolve-HistoryTargetAddress` when falling back to the last known IP of a target to prevent connecting to a different physical host.
...
- Fixed hostname resolution and device visibility in the LAN connection selector:
  ...
  - Added a persistent JSON hostname cache (`hosts-cache.json`) and an automatic background resolution job (`Start-DeviceCheckBackgroundResolver`) to query and cache slow NetBIOS/LLMNR names (which can take over 4.5 seconds for devices with multiple network interfaces like the boss PC) in the background without introducing UI scan delays, providing instant hostname display on subsequent scans.
...
- Fixed full snapshot property collection under `Set-StrictMode` by reading optional `Get-PnpDeviceProperty` fields through safe property access, so full archive samples retain per-device property groups instead of dropping them on devices/properties without `Data`.
