...
- Added network scan phase benchmarking inside `Get-DeviceCheckDiscoveredHosts` that writes detailed phase durations into date-time stamped log files inside a dedicated `logs` folder (ignored in git), dynamically reading the most recent log file to render results.
