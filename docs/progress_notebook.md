...
9. **Targeted Subnet Scanning & Async NetBIOS:** Restored scan speed (<1.5s) by restricting ping and TCP scans only to neighbor/history targeted IPs instead of all 254 subnet IPs. Implemented parallel async `GetHostEntryAsync` requests with a 400ms timeout for online hosts, enabling NetBIOS name resolution (e.g. `DESKTOP-GU5851U`) without blocking the main thread. Non-WinRM/non-SMB online devices (like the boss PC) are now successfully discovered via ICMP ping response mapping and displayed as `(WinRM Disabled)`. Wrapped all task `WaitAll` calls in try/catch blocks to absorb `AggregateException` errors.
