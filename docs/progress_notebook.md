...
9. **Subnet Ping and NetBIOS Fallback:** Refactored subnet scanning to run concurrent pings across all 254 subnet IPs in parallel. This forces the OS ARP cache to refresh dynamically, resolving cached state lags (such as WinRM online status needing a restart). Wrapped the `.NET` `WaitAll` call in a try/catch block to absorb `AggregateException` errors when individual ping requests throw exceptions. Also added `GetHostEntry` NetBIOS lookup fallback for online-confirmed hosts to resolve machine names (e.g. `DESKTOP-GU5851U`), and enabled detection of active devices without WinRM/SMB open (e.g. boss computer) via ICMP echo response tracking.
