...
8. **Stale Log Prevention:** Prevented rendering stale log files from previous sessions when toggling Benchmark Mode. The script now records `$script:ScriptStartTime` upon launch and filters local scan logs, only reading log files that were generated during the current session. Timings for scans performed in the current session (even when Benchmark Mode was OFF) are cached in-memory and rendered immediately when toggled ON.
